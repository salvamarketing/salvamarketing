...
const InteractiveNeuralVortex = ({ className = "" }: { className?: string }) => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        noise += pow(noise, 10.);
        noise = max(.0, noise - .2); // Adjusted threshold for more visibility
        vec3 color = vec3(1.0, 0.45, 0.0) * noise; // Solid vibrant orange scaled by noise
