    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (cardsRef.current.length) {
        ...
        for (let j = i + 1; j < cardsRef.current.length; j++) {
          ...
          const startOverlap = jPinStart - containerHeight * 0.4;
          const jOverlapProgress = calculateProgress(scrollTop, startOverlap, jPinStart);
          continuousDepth += jOverlapProgress;
        }

        if (continuousDepth > 0) {
          if (blurAmount) blur = Math.max(0, continuousDepth * blurAmount);
          opacity = Math.max(0.2, 1 - continuousDepth * 0.25);
          brightness = Math.max(0.1, 1 - continuousDepth * 0.3);
        }
      }
      ...
      const hasChanged =
        ...
        Math.abs(lastTransform.brightness - newTransform.brightness) > 0.05;

      if (hasChanged) {
        const transform = `translate3d(0, ${newTransform.translateY}px, 0) scale(${newTransform.scale}) rotate(${newTransform.rotation}deg)`;
        let filter = newTransform.blur > 0 ? `blur(${newTransform.blur}px)` : '';
        if (newTransform.brightness < 1) {
          filter += filter ? ` brightness(${newTransform.brightness})` : `brightness(${newTransform.brightness})`;
        }
